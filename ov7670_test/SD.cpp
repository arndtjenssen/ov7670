...
boolean SDClass::begin(uint8_t csPin) {
  ...
  //return card.init(SPI_HALF_SPEED, csPin) && volume.init(card) && root.openRoot(volume);
	return card.init(SPI_FULL_SPEED, csPin) && volume.init(card) && root.openRoot(volume);
	//return card.init(SPI_QUARTER_SPEED, csPin) && volume.init(card) && root.openRoot(volume);
}
