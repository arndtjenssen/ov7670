...
void Adafruit_ST7735::commonInit(const uint8_t *cmdList) {
  ...
  if(hwSPI) { // Using hardware SPI
    ...
    SPI.setClockDivider(SPI_CLOCK_DIV2); // 8MHz (full speed)
    //SPI.setClockDivider(SPI_CLOCK_DIV8); // 2Mhz (quarter speed)
#endif
...
void Adafruit_ST7735::displayOff(void) {
	writecommand(ST7735_DISPOFF);
	//delay(150);
}

void Adafruit_ST7735::displayOn(void) {
